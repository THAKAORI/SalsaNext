...
def send_line_notify(notification_message):
    ...
    LINEに通知する
    """
    line_notify_token = '7eW4xgJWK0lLHTQpwLtMuiTh0nb9ic3VneqQtfPGnzc'
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None,uncertainty=False):
        ...
        print("batch_size, channel, height, width", 20, 5, 64, 2048)
        print(summary(self.model, (20, 5, 64, 2048)))
        ...
        with torch.no_grad():
            ...
            if self.uncertainty:
                ...
                for i, jacc in enumerate(class_jaccard):
                    print('IoU class {i:} [{class_str:}] = {jacc:.3f}'.format(
                        i=i, class_str=class_func(i), jacc=jacc))
                    message = ('IoU class {i:} [{class_str:}] = {jacc:.3f}'.format(
                         i=i, class_str=class_func(i), jacc=jacc))
                    send_line_notify(message)
                    save_to_log(self.log, 'log.txt', 'IoU class {i:} [{class_str:}] = {jacc:.3f}'.format(
                        i=i, class_str=class_func(i), jacc=jacc))
